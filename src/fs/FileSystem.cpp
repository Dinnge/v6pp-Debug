...
Inode* FileSystem::IAlloc(short dev)
{
	...
	while(true)
	{
		...
		if(0 == pNode->i_mode)
		{
			...
			/* 设置SuperBlock被修改标志 */
			sb->s_fmod = 1;
			fs_trace_log2("inode_alloc", dev, ino);
			return pNode;
		}
		else	/* 如果该Inode已被占用 */
		{
...
void FileSystem::IFree(short dev, int number)
{
	...
	/* 设置SuperBlock被修改标志 */
	sb->s_fmod = 1;
	fs_trace_log2("inode_free", dev, number);
}
...
Buf* FileSystem::Alloc(short dev)
{
	...
	fs_trace_log2("block_alloc", dev, blkno);
...
void FileSystem::Free(short dev, int blkno)
{
	...
	sb->s_fmod = 1;
	fs_trace_log2("block_free", dev, blkno);
}
